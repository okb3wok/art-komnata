...
//https://github.com/tristangoossens/php-sitemap-generator/blob/main/sitemap-generator.php
...
class SitemapGenerator
{
  ...
  public function crawlPage($page_url)
  {
    ...
    foreach ($anchors as $a) {
      ...
      if (!$this->config['ALLOW_ELEMENT_LINKS']) {
        ...
        if (substr($next_url, 0, 1) == "#" || $next_url == "/" || substr($next_url, 0, 2) == "/#") {
          continue;
        }
      }
      ...
      if (!$found && !str_starts_with($next_url, 'tel:') && !str_ends_with($next_url, '.jpg')) {
  ...
  public function convertRelativeToAbsolute($page_base_url, $link)
  {
    $first_character = substr($link, 0, 1);
    if ($first_character == "?" || $first_character == "#") {
      return $page_base_url . $link;
    } else if(str_starts_with($link, 'tel:')){
      return $link;
    }else if ($first_character == ".") {
      return $this->site_url_base . substr($link, 1, strlen($link) - 1);
    }else if ($first_character != "/") {
